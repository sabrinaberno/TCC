...
\paragraph{Tentativas preliminares sem desempenho satisfatório.} \mbox{}\\
Durante o processo de desenvolvimento do modelo final, foram testadas diferentes abordagens de aprendizado de máquina que, apesar de metodologicamente corretas, não apresentaram desempenho satisfatório.

\begin{itemize}
    \item \textbf{Primeira tentativa — Regressão com Gradient Boosting:} Foi testado um modelo de regressão com \textit{Gradient Boosting} para prever a porcentagem de euploidia. Apesar da utilização de \textit{early stopping} e validação cruzada com busca em grade, os resultados foram insatisfatórios. O coeficiente de determinação no teste ($R^2$) foi de apenas 0{,}05, com erro absoluto médio (MAE) de 7{,}49.

    \item \textbf{Segunda tentativa — MLP simples sem aumento de dados:} Foi utilizada uma \textit{Rede Neural MLP} com arquitetura básica (camadas ocultas de 32 e 16 neurônios) e normalização dos dados com \texttt{StandardScaler}. O modelo apresentou acurácia de 65\% e \textit{recall} de apenas 0{,}38 para a classe euploide, evidenciando baixa sensibilidade. 

    \item \textbf{Terceira tentativa — MLP com aumento de dados por ruído gaussiano:} Nesta abordagem, se aplicou \textit{data augmentation} nas variáveis numéricas do conjunto de treino. Mesmo com até 50 tentativas utilizando diferentes \texttt{random states}, nenhuma execução obteve desempenho aceitável. A acurácia não passou de 29\% e os \textit{recalls} ficaram abaixo de 0{,}33, com AUC muito baixa. A hipótese é que o ruído inserido comprometeu a estrutura dos dados reais.
\end{itemize}
...
As práticas de \textbf{Clean Code} foram aplicadas ao longo de todo o desenvolvimento da aplicação, com base nos princípios descritos por \citeonline{martin2009cleancode}. Tais práticas visam garantir legibilidade, manutenibilidade e robustez do código, aspectos essenciais em soluções que envolvem dados sensíveis e contextos clínicos. Dentre os princípios adotados, destacam-se:
